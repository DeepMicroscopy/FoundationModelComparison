...
from transformers import ViTModel, SwinForImageClassification, AutoModel
import os
...
class Classifier(nn.Module):
    ...
    def load_model(self, model_name: str, lora: bool = False, num_classes: int = 2) -> Tuple[nn.Module, Compose]:
        ...
        model_map = {
            ...
            "ViT_B": lambda: (ViTForClassification(model_name="ViT_B", num_classes=num_classes), ViTForClassification.get_transforms("ViT_B")),
            "ViT_L": lambda: (ViTForClassification(model_name="ViT_L", num_classes=num_classes), ViTForClassification.get_transforms("ViT_L")),
            "ViT_S_DINOv3": lambda: (ViTForClassification(model_name="ViT_S_DINOv3", num_classes=num_classes), ViTForClassification.get_transforms("ViT_S_DINOv3")),
    ...
    def initialize_classification_head(self, model_name: str, model: nn.Module, num_classes: int = 2) -> nn.Module:
        ...
        elif model_name == 'ViT_tiny' or model_name == 'ViT_S' or model_name == 'Swin_base' or model_name == 'convnext_base' or 'efficientnet' in model_name or 'densenet' in model_name:
            return model 
        else:
