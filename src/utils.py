...
from transformers import ViTModel, SwinForImageClassification, ConvNextForImageClassification, AutoModel, AutoImageProcessor
...
def return_forward(model_name: str) -> callable:
    ...
    elif model_name in  ['phikon', 'ViT_S_DINOv3']:
        def forward_fn(model, batch, remaining):
...
def load_model_and_transforms(model_name: str) -> tuple:
    ...
    if model_name == 'uni':
        ...
        model = timm.create_model('vit_tiny_patch16_224.augreg_in21k', pretrained=True, num_classes=1)
        ...
        model = SwinForImageClassification.from_pretrained(
                    ...
                    num_labels=1,  # Set to your task's number of classes
        ...
        model = timm.create_model('efficientnet_b0.ra_in1k', pretrained=True, num_classes=1)
        data_config = timm.data.resolve_model_data_config(model)
        transforms = timm.data.create_transform(**data_config, is_training=False)

    elif model_name == "efficientnet_b3":
        model = timm.create_model('efficientnet_b3.ra2_in1k', pretrained=True, num_classes=1)
        data_config = timm.data.resolve_model_data_config(model)
        transforms = timm.data.create_transform(**data_config, is_training=False)
    
    elif model_name == "efficientnet_b7":
        model = timm.create_model('tf_efficientnet_b7.ra_in1k', pretrained=True, num_classes=1)
        transforms = T.Compose([
            T.Resize(224),
            T.ToTensor(),
            T.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
        ...
        model = timm.create_model('densenet121.ra_in1k', pretrained=True, num_classes=1)
        data_config = timm.data.resolve_model_data_config(model)
        transforms = timm.data.create_transform(**data_config, is_training=False)

    elif model_name == "convnext_base":
        model = ConvNextForImageClassification.from_pretrained("facebook/convnext-base-224-22k", num_classes=1)
        transforms = T.Compose([
            T.Resize(224),
            T.ToTensor(),
            T.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
        ...
        model = timm.create_model('vit_small_patch16_224.augreg_in21k', pretrained=True, num_classes=1)
        data_config = timm.data.resolve_model_data_config(model)
        transforms = timm.data.create_transform(**data_config, is_training=False)
